...
@artist_bp.route("/orders", methods=["GET"])
@token_required
@role_required("artist")
def list_orders():
    """List orders for the authenticated artist."""
    artist_id = _get_artist_id()
    try:
        limit = int(request.args.get("limit", 50))
        skip = int(request.args.get("skip", 0))
    ...
    docs = service.list_orders_by_artist(artist_id, limit=limit, skip=skip)
    return jsonify({"success": True, "orders": docs}), 200


@artist_bp.route("/orders/<order_id>/ship", methods=["POST"])
@token_required
@role_required("artist")
def ship_order(order_id):
    ...
    try:
        service = OrderService(OrderRepository())
        result = service.ship_order(order_id, artist_id)
        ...
        return jsonify({"success": False, "message": "Order shipping failed."}), 500
...
def complete_order(order_id):
    """Legacy endpoint - now marks as shipped instead of completed."""
    artist_id = _get_artist_id()
    try:
        service = OrderService(OrderRepository())
        result = service.complete_order(order_id, artist_id)  # This now calls ship_order
        return jsonify(result), 200
    except OrderNotFoundError as e:
        return jsonify({"success": False, "message": str(e)}), 404
    except UnauthorizedOrderActionError as e:
